...
## Astro 6 + `@astrojs/cloudflare` 13+: custom `main` in `wrangler.jsonc` is silently ignored — patch the redirected config

- **Context**: Wiring `@sentry/cloudflare` (`Sentry.withSentry(...)`) as a wrapper entry point for the Astro-generated Cloudflare Worker. The natural first attempt (per the Sentry brief and adapter docs) is to set `main: "./sentry.server.config.ts"` in the root `wrangler.jsonc`.
- **Problem**: `@astrojs/cloudflare` 13+ uses a **redirected configuration** — the adapter writes `dist/server/wrangler.json` at build time and points wrangler at *that* file (visible in wrangler output: `Using redirected Wrangler configuration. Configuration being used: "dist\server\wrangler.json". Original user's configuration: "wrangler.jsonc"`). The redirected config always has `main: "entry.mjs"` (the raw adapter entry), so any `main` override in root `wrangler.jsonc` is silently discarded. The deployed Worker runs the unwrapped entry — Sentry never initialises, DSN is never read, and no events reach the dashboard. Dry-runs and builds pass without warning; the failure is silent until you inspect the deployed bundle or notice missing events.
- **Rule**: When a custom Workers entry point is needed on Astro 6 + `@astrojs/cloudflare` 13+, do NOT rely on `main` in `wrangler.jsonc`. Instead: (a) add a `postbuild` script that runs after `astro build`, (b) generate the wrapper file directly into `dist/server/`, bundling all non-Workers-native deps with esbuild (mark `cloudflare:*`, `node:*`, and the adapter's `entry.mjs` + `./chunks/*` as `external`), and (c) patch `dist/server/wrangler.json` to point `main` at your wrapper. Verify by grepping the built bundle for the wrapper's imports and by inspecting `dist/server/wrangler.json` after every build. See `scripts/patch-sentry-entry.mjs` for the canonical implementation.
- **Applies to**: plan, implement, impl-review
