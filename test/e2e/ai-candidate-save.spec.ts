...
test.describe("Risk #2 — AI candidates: only the accepted subset (with edits) persists; rejected never hits DB", () => {
  test("accept one edited + one unedited, reject a third: reload shows exactly the two accepted with the edit intact", async ({
    ...
    const acceptedEditedId = acceptedEditedBody.data.id;
    const acceptedOriginalId = acceptedOriginalBody.data.id;
    ...
    // CLEANUP: delete the two accepted rows via the real DELETE API so the DB
    // is left exactly as we found it (matches seed.spec.ts's cleanup style).
    // We fire the DELETE from inside the page (via page.evaluate + fetch) so
    // the request carries the exact same session cookie the UI would use —
    // page.request's APIRequestContext returned 403 here, likely because the
    // Astro handler's cookie decode expects the browser's Origin/CORS headers
    // that APIRequestContext does not replicate 1:1.
    for (const id of [acceptedEditedId, acceptedOriginalId]) {
      const status = await page.evaluate(async (flashcardId) => {
        const response = await fetch(`/api/flashcards/${flashcardId}`, { method: "DELETE" });
        return response.status;
      }, id);
      expect(status, `cleanup: delete of ${id} should succeed`).toBe(204);
    }
  });
});
